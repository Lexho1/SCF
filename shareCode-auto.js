...
const shareCodes = [
    {
        ...
        jdzz: "AUWE5m6qSmDYKCTWq1HpDwQ",
        jdcrazyjoy: "Bs_l5uxPbj8nyidQtwltlat9zd5YaBeE",
    },
    {
        name: "Lexho1",
        ...
        jdzz: "ACjJSnq-SxDAJD33_33VJnXc",
        jdcrazyjoy: "FV5dfQMJ92NHJTm9KvYOg2XGvf5Si_Xe",
    },
];
$.result = [];
let s = 500 //各数据接口延迟
...
!(async () => {
    for (let i = 0; i < shareCodes.length; i++) {
        const { name, bean, farm, pet, ddfactory, jxfactory, jdzz, jdcrazyjoy } = shareCodes[i];
        message += `${name}：`;
        ...
        jdzz && await createShare('jdzz',jdzz)// 京东赚赚
        jdcrazyjoy && await createShare('jdcrazyjoy',jdcrazyjoy)// 疯狂的joy
        message += '\n';
    }
    await showMsg()
})()
...
function createShare(type,share) {
    return new Promise((resolve, reject) => {
        ...
        let http = "http://api.turinglabs.net";
        if(type == "jdcrazyjoy" || type == "jdzz"){
            http = "https://code.chiang.fun";
        }
        setTimeout(() => {
            const url = {
                url: `${http}/api/v1/jd/${type}/create/${share}/`,
            }
            $.get(url, (err, resp, data) => {
                const _data = JSON.parse(data)
                if(_data.message){
                    $.log(`${nickName} ${type}: ${_data.message}`);
                }else{
                    $.log(`${nickName} ${type}: ${_data.msg}`);
                }
                let msg = '‼️';
                if (_data) {
                    if(_data.code == "400"){
